...
sub clean_database {
    ...
    # Custom: Invalidate cache once after batch deletion
    invalidate_cache() if ($deleted_arcs > 0 || $unlinked_arcs > 0);
...
sub invalidate_cache ( $rebuild_indexes = 0 ) {
    ...
    # Custom: Add a 5-minute rate limit for cache invalidation to prevent performance issues
    # during frequent operations (like mass imports or metadata updates)
    my $last_clear = $redis->hget( "LRR_SEARCHCACHE", "created" ) || 0;
    my $now = time;

    if ( ($now - $last_clear) < 300 && !$rebuild_indexes ) {
        $logger->debug("Cache was cleared less than 5 minutes ago, skipping full wipe.");
        $redis->quit();
        return;
    }

    # Custom: Instead of deleting everything, we can do a "smart refresh"
    # where we only regenerate the most common search entries (like empty searches)
    # and remove everything else.
    # NOTE: With 90k+ archives, each search_uncached call takes multiple seconds,
    # and recomputing up to 10 of them synchronously blocks the calling worker
    # for minutes. We now wipe the cache (entries are rebuilt lazily on the next
    # search) and pre-warm the default homepage query in the background via Minion.
    my $key_count = $redis->hlen("LRR_SEARCHCACHE");
    $logger->info("Invalidating search cache ($key_count entries)...");
    $redis->del("LRR_SEARCHCACHE");
    # Also invalidate the full archive list cache
    $redis->del("LRR_ARCLIST_CACHE");
    # Invalidate the precomputed title sort index (will be rebuilt lazily)
    $redis->del("LRR_SORTED_title");
    # Re-create the hash so the rate-limit timestamp above survives the DEL
    $redis->hset( "LRR_SEARCHCACHE", "created", time );
    $redis->quit();

    # Asynchronously pre-warm the default homepage query (empty filter, date_added desc)
    # so the first load after an invalidation stays fast.
    LANraragi::Model::Config->get_minion->enqueue( rebuild_search_cache => [ "--date_added-1-0-0-0-0" ] => { priority => 1 } );

    # Asynchronously rebuild the precomputed title sort index (P1-A)
    # so the first title-sorted search after an invalidation stays fast.
    LANraragi::Model::Config->get_minion->enqueue( rebuild_title_sort_index => [] => { priority => 2 } );
