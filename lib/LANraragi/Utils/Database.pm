...
sub invalidate_cache ( $rebuild_indexes = 0 ) {
    ...
    my $redis  = LANraragi::Model::Config->get_redis_search;
    my $logger = get_logger( "Cache", "lanraragi" );

    # Rate limiting: only allow one call per 5 minutes
    my $rate_limit_key = "LRR_CACHE_INVALIDATE_LAST_CALL";
    my $current_time   = time;
    my $last_call_time = $redis->get($rate_limit_key) || 0;
    my $time_diff      = $current_time - $last_call_time;

    if ( $time_diff < 300 ) {    # 300 seconds = 5 minutes
        my $remaining_time = 300 - $time_diff;
        $logger->info("[CACHE_INVALIDATE] Skipping cache invalidation - rate limited. Next call allowed in ${remaining_time} seconds.");
        $redis->quit();
        return;
    }

    # Update the last call timestamp
    $redis->set( $rate_limit_key, $current_time );
    $redis->expire( $rate_limit_key, 300 );    # Auto-expire after 5 minutes

    $logger->info("[CACHE_INVALIDATE] Starting cache invalidation process.");

    # Get all keys in the search cache hash
    my @cache_keys = $redis->hkeys("LRR_SEARCHCACHE");

    # Process all cache entries except 'created'
    foreach my $key (@cache_keys) {
        next if $key eq "created";

        # Decode the cache key to extract search parameters
        my $decoded_key = LANraragi::Utils::Redis::redis_decode($key);
        my @params      = split( /-/, $decoded_key, 7 );

        # Skip if the key format is not as expected
        next if scalar(@params) != 7;

        my ( $category_id, $filter, $sortkey, $sortorder, $newonly, $untaggedonly, $grouptanks ) = @params;

        # Only refresh cache entries with empty filter, remove others
        if ( $filter eq "" ) {
            $logger->debug("Refreshing cache for key: $decoded_key");

            # Import search_uncached function from Search model
            require LANraragi::Model::Search;

            eval {
                # Regenerate the search results
                my ( $keyed_count, @filtered ) = LANraragi::Model::Search::search_uncached(
                    $category_id, $filter, $sortkey, $sortorder,
                    $newonly, $untaggedonly, $grouptanks
                );

                # Update the cache with fresh data, prepending the keyed count for partition-aware cache inversion
                $redis->hset( "LRR_SEARCHCACHE", $key, nfreeze [ $keyed_count, @filtered ] );
            };

            if ($@) {
                $logger->warn("Failed to refresh cache for key $decoded_key: $@");

                # Remove the problematic cache entry
                $redis->hdel( "LRR_SEARCHCACHE", $key );
            }
        } else {
            $logger->debug("Removing cache entry with non-empty filter: $decoded_key");
            $redis->hdel( "LRR_SEARCHCACHE", $key, );
        }
    }

    # Update the created timestamp to mark cache refresh
