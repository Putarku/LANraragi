...
### **Custom Merge Information**
基于官方 0.9.70 版本合并私有改动：
- **Search Engine Optimization**: 移植了 0.9.5 定制版中的 Lua 脚本优化，显著提升了搜索（Pagecount/Progress/Tags/Titles）的性能。
- **UI Customization**: 根据定制需求，在首页 Carousel 菜单中禁用了 `On Deck`、`Inbox` 和 `Untagged` 选项。
- **Debugging & Reliability**: 在搜索模块中增加了详细的调试日志，并为所有搜索参数添加了默认值初始化，防止未定义变量引起的警告。
- **Compatibility**: 所有定制逻辑已适配 0.9.70 的新架构（如 `LANraragi::Utils::Redis` 命名空间及 OpenAPI 响应格式）。
